...
const TG_ICON = ICONS.telegram;
function platformIcon(p) {{ return ICONS[p] || ICONS.telegram; }}
const PLATFORM_COLORS = {{telegram:'#2AABEE',instagram:'#E1306C',youtube:'#FF0000',x:'#000000'}};
...
function dmLink(ch) {{
  const url = ch.dm_url || ch.profile_url;
  if (!url && !ch.admin_username) return '<span class="no-admin">No contact found</span>';
  const p = ch.platform || 'telegram';
  const labels = {{telegram:'DM',instagram:'DM',youtube:'Contact',x:'DM'}};
  const adminText = ch.admin_username ? ` @${{ch.admin_username}}` : '';
  const href = url || (p === 'telegram' ? `https://t.me/${{ch.admin_username}}` : '#');
  const color = PLATFORM_COLORS[p] || 'var(--accent)';
  return `<a href="${{href}}" target="_blank" rel="noopener" class="admin-link" style="background:${{color}}">${{platformIcon(p)}}${{labels[p]}}${{adminText}}</a>`;
}}
function profileLink(ch) {{
  const p = ch.platform || 'telegram';
  const url = ch.profile_url || (p === 'telegram' ? `https://t.me/s/${{ch.telegram_username}}` : '#');
  return `<a href="${{url}}" target="_blank" rel="noopener" class="channel-link">${{platformIcon(p)}}View Profile</a>`;
}}
    ...
    return `
      <div class="card outreach-card">
        <div class="outreach-header">
          <div class="channel-name"><span style="opacity:0.6;margin-right:4px">${{platformIcon(ch.platform||'telegram')}}</span>${{ch.title || '@'+(ch.username||ch.telegram_username)}}</div>
          <div class="channel-meta">${{fmt(ch.subscriber_count)}} subs &middot; ${{ch.language || '?'}} &middot; @${{ch.username||ch.telegram_username}}</div>
        </div>
